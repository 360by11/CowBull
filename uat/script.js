            ...
            <div class="history-row">
                <div class="guess-content">
                    <div class="guess-value">
                        <span class="blink-red">Game Over! The Secret was : ${secretCode}</span>
                    </div>
                </div>
            </div>`;
...
function toggleDailyNumber() {
    ...
    if (attempts > 0 && !end) {
        if (!confirm('Changing game mode will start a new game. Current progress will be lost. Continue?')) {
            ...
            document.getElementById('game-mode').value = isNumberOfDay ? 'daily' : 'normal';
            return;
        }
    }
    const modeSelect = document.getElementById('game-mode');
    isNumberOfDay = modeSelect.value === 'daily';
    startNewGame();
}
